...
module.exports = function (router, _myData) {
    ...
    router.post('/search-criteria-answer', function (request, response) {
        ...
        if (searchCriteria == "int-sr-cusName") {
            response.redirect("/v16/internal-search-customer-name")
        } else {
            response.redirect("/v16/internal-search")
        }
    })
    ...
    router.post('/remove-yourself-answer', function(request, response) {

        var removeYourself = request.session.data['remove-yourself']
        if (removeYourself == "yes"){
            response.redirect("/v16/remove-yourself-confirmation")
        } else {
    ...
    router.post('/remove-person-answer', function(request, response) {
    ...
    // v16 - add-person-permissions-land-details

    router.post('/add-person-permissions-answer', function(request, response) {

        var permissionsBusinessDetails = request.session.data['permissions-business-details']
        if (permissionsBusinessDetails == "Full permission"){
            response.redirect("/v16/add-person-check-answers")
        } else {
    ...
    //business details - change bank details - country (ba02) removed from the v16
    //journey (external and internal). The view is retained at
    //app/views/v16/business-details-bank-country.html but is no longer routed to.
    ...
    router.post('/v16-validation-answer', function(request, response) {
        ...
        if (validationAnswer == "No" && bankType == "Business"){
            response.redirect("v16/business-details-bank-check-uk-business")
        }
