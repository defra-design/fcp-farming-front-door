module.exports = function (router, _myData) {
    ...
    router.post('/register-ab-type-answer', function (request, response) {
        ...
        if (RABType == "I want to apply for, manage and receive rural payments") {
            response.redirect("v14/register-ab-business-legal-status")
        }
        else {
            response.redirect("v14/register-ab-business-name")
        }
    })
    ...
    router.post('/register-ab-vat-answer', function (request, response) {
        ...
        if (RABVAT == "Yes") {
            response.redirect("v14/register-ab-business-VAT")
        }
        else {
            response.redirect("v14/register-ab-business-name")
        }
    ...
    router.post('/send-answer-v14', function (request, response) {

        var sendAnswer = request.session.data['what-to-send']
        if (sendAnswer == "Complaint evidence") {
            response.redirect("v14/doc-upload-ref-complaint")
        }
    ...
    router.post('/CS-answer-v14', function (request, response) {

        var CSAnswer = request.session.data['doc-upload-cs-tag']
        if (CSAnswer == "Capital claim") {
            response.redirect("v14/doc-upload-ref-cs-agreement")
        }
        if (CSAnswer == "Capital grant application") {
            response.redirect("v14/doc-upload-cs-tag-app")
        }
        if (CSAnswer == "Agreement") {
            response.redirect("v14/doc-upload-cs-tag-agreement")
        }
        if (CSAnswer == "Facilitation fund") {
            response.redirect("v14/doc-upload-cs-tag-ff")
        }
        if (CSAnswer == "Revenue claim") {
            response.redirect("v14/doc-upload-cs-tag-revenue")
        }
        else {
            response.redirect("_common/not-testing")
        }
